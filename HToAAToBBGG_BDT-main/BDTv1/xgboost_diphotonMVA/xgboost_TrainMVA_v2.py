...
def loadSignal(wSig):
  ...
  df_sig = uproot.open("/home/anirban/HToAAToBBGG_BDT-main/training_root_files/WH_all_mA_2018_bdt_v1.root")['tree'].pandas.df()
  df_sig['weight_pure'] = df_sig['evt_wgt']
...
def loadMCBackground(scaleQCD):
  ...
  df_TTGJets = uproot.open('/home/anirban/HToAAToBBGG_BDT-main/training_root_files/TTGJetsV2_2018_bdt_v1.root')['tree'].pandas.df()[vars]
  
  df_DY = uproot.open('/home/anirban/HToAAToBBGG_BDT-main/training_root_files/DYJetsToLLM50V2_2018_bdt_v1.root')['tree'].pandas.df()[vars]
  
  df_TTSL = uproot.open('/home/anirban/HToAAToBBGG_BDT-main/training_root_files/TTSLV2_2018_bdt_v1.root')['tree'].pandas.df()[vars]

  df_TTTo2L2Nu = uproot.open('/home/anirban/HToAAToBBGG_BDT-main/training_root_files/TTTo2L2NuV2_2018_bdt_v1.root')['tree'].pandas.df()[vars]


  df_bkg = df_TTGJets
  df_bkg = pd.concat((df_bkg,df_DY))
  df_bkg = pd.concat((df_bkg,df_TTSL))
  df_bkg = pd.concat((df_bkg,df_TTTo2L2Nu)) 
...
if __name__ == '__main__':
  ...
  df_sig['weight_noWSig'] = df_sig['evt_wgt']
  df_bkg['weight_noWSig'] = df_bkg['evt_wgt']
  
  #Apply additional weights
  if wSig:
    df_sig['weight'] = df_sig['evt_wgt'] * df_sig['weight_wSig']
  if negW0:
  ...
  for var in ['weight_pure','weight_wSig','weight_posRatio','weight_absRatio','isTest']:
    if var not in treeVars:
      treeVars.append(var) 
  makeTree(X_sig,treeVars,'Sig','Sig'+label+'.root')
